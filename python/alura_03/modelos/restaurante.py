...
Estado: {self.ativo}"""
    ...
    def adicionar_ao_cardapio(self, item):
        if isinstance(item, Bebida):
            self._cardapio["bebidas"].append(item)
        elif isinstance(item, Prato):
            self._cardapio["pratos"].append(item)
        elif isinstance(item, Sobremesa):
            self._cardapio["sobremesas"].append(item)

    def mostrar_cardapio(self):
        print(f"Cadápio do restaurante {self._nome}\n".upper())
        for tipo_de_item in self._cardapio.keys():
            print(tipo_de_item.title())
            for i, item in enumerate(self._cardapio[tipo_de_item], start=1):
                print(f"{str(i).rjust(2)}. {item}")
                if hasattr(item, "_descricao"):
                    print(f"{''.ljust(4)}Descrição: {item._descricao}")
                if hasattr(item, "_tamanho"):
                    print(f"{''.ljust(4)}Tamanho: {item._tamanho}")
            print()
